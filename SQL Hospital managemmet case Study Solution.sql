...
DROP TABLE Appointment IF EXISTS Appointment;
...
INSERT INTO Appointment VALUES(86213939,100000004,102,9,TO_DATE('2008-04-27 10:00', 'YYYY-MM-DD HH24:MI'),TO_DATE('2008-04-27 11:00', 'YYYY-MM-DD HH24:MI'),'A');
...
DROP TABLE Prescribes IF EXISTS Prescribes;
...
DROP TABLE On_Call IF EXISTS On_Call;
...
--pending-----------------------------------------------------------------------------------
DROP TABLE Stay IF EXISTS Stay;
CREATE TABLE Stay (
...
DROP TABLE Undergoes  IF EXISTS Undergoes;
...
-- 8.2 Same as the previous query, but include the following information in the results: Physician name, name of procedure, date when the procedure was carried out, name of the patient the procedure was carried out on.
 select P.NAME ,PR.NAME PROCEDURES, U.DATEUNDERGOES, PT.NAME As Patient_name
 FROM PHYSICIAN P LEFT JOIN UNDERGOES U ON(P.EMPLOYEEID=U.PHYSICIAN)
...
-- 8.3 Obtain the names of all physicians that have performed a medical procedure that they are certified to perform, but such that the procedure was done at a date (Undergoes.Date)
--after the physician's certification expired (Trained_In.CertificationExpires).
SELECT P.NAME PHYSICIAN_NAME 
FROM PHYSICIAN P INNER JOIN UNDERGOES U ON(P.EMPLOYEEID=U.PHYSICIAN)
INNER JOIN TRAINED_IN T ON(T.PHYSICIAN=U.PHYSICIAN AND U.PROCEDURES=T.TREATMENT )
...
SELECT P.NAME PHYSICIAN_NAME ,PR.NAME PROCEDURES,U.DATEUNDERGOES,T.CERTIFICATIONEXPIRES, PT.NAME As Patient_name
FROM PHYSICIAN P INNER JOIN UNDERGOES U ON(P.EMPLOYEEID=U.PHYSICIAN)
INNER JOIN TRAINED_IN T ON(T.PHYSICIAN=U.PHYSICIAN AND U.PROCEDURES=T.TREATMENT )
INNER JOIN PATIENT PT ON (U.PATIENT=PT.SSN)
LEFT JOIN PROCEDURES PR ON (U.PROCEDURES=PR.CODE)
...
SELECT PT.NAME PATINET ,A.PHYSICIAN ,PH.NAME PHYSICIAN_NAME, N.NAME ,STARTDATE,ENDDATE,EXAMINATIONROOM FROM PATIENT PT
INNER JOIN APPOINTMENT A ON (PT.SSN=A.PATIENT)
...
INNER JOIN ROOM R ON (R.BLOCKCODE=O.BLOCKCODE)
WHERE 
ROOMNUMBER=123
...
-- 8.9 Obtain the names of all patients (also include, for each patient, the name of the patient's primary care physician), such that \emph{all} the following are true:
   ...
   SELECT 
   P.NAME,
   PH.NAME primary_care_physician 
   FROM PATIENT P LEFT JOIN PHYSICIAN PH ON (P.PCP=PH.EMPLOYEEID)
    WHERE 
-- The patient has been prescribed some medication by his/her primary care physician.
    EXISTS
    (SELECT * FROM  prescribes PC 
           where pc.patient=p.ssn AND
           PC.PHYSICIAN=P.PCP)
    
-- The patient has undergone a procedure with a cost larger that $5,000
   OR  EXISTS
            (SELECT * FROM UNDERGOES U LEFT JOIN PROCEDURES PR  ON (U.PROCEDURES=PR.CODE)
             WHERE U.PATIENT =P.SSN AND PR.COST>5000)
             
 -- The patient has had at least two appointment where the nurse who prepped the appointment was a registered nurse.
   OR 2 <=( SELECT COUNT(APPOINTMENTID) FROM  APPOINTMENT A ,NURSE N WHERE
                A.PATIENT=P.SSN AND A.PREPNURSE=N.EMPLOYEEID AND N.REGISTERED=1)
-- The patient's primary care physician is not the head of any department.

    OR  NOT EXISTS (SELECT * FROM DEPARTMENT D WHERE
                          D.HEAD=P.PCP)
